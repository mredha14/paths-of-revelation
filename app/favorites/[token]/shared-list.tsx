...
export default function SharedList({ shareToken }: { shareToken: string }) {
  ...
  useEffect(() => { const map = mapInstance.current, L = leaflet.current?.default; if (!map || !L || !shared) return; markers.current?.remove(); const layer = L.layerGroup().addTo(map); markers.current = layer; shared.places.forEach((place) => { const marker = L.circleMarker([place.lat, place.lng], { radius: place.id === selected?.id ? 12 : 9, color: "#fff", weight: 3, fillColor: place.id === selected?.id ? "#ba8132" : "#315e4c", fillOpacity: 1 }).addTo(layer); marker.bindTooltip(ar ? place.title : place.titleEn, { direction: "top", offset: [0, -8], opacity: .94 }); marker.on("click", () => { setSelected(place); map.flyTo([place.lat, place.lng], Math.max(map.getZoom(), 14), { duration: .65 }); }); }); }, [ar, mapReady, shared, selected]);
  ...
  return <main className="site" dir={ar ? "rtl" : "ltr"}>
    ...
    {error ? <section className="shared-empty"><h1>{ar ? "القائمة غير متاحة" : "List unavailable"}</h1><p>{error}</p><a href="/">{ar ? "العودة إلى الخريطة" : "Back to the map"}</a></section> : <section className="workspace">
      ...
      <article className="card">{selected ? <><img src={selected.photo} alt={text(selected, "title")}/><div><p>{selected.city === "Makkah" ? (ar ? "مكة المكرمة" : "MAKKAH") : (ar ? "المدينة المنورة" : "MADINAH")} · {text(selected, "type")}</p><section><h2>{text(selected, "title")}</h2></section><p className="description">{text(selected, "description")}</p><nav><a target="_blank" rel="noreferrer" href={`https://www.google.com/maps/dir/?api=1&destination=${selected.lat},${selected.lng}`}>{ar ? "الاتجاهات" : "Directions"} ↗</a></nav></div></> : <div className="shared-card-empty">{ar ? "اختر مكاناً من القائمة." : "Choose a place from the list."}</div>}</article>
    </section>}
  </main>;
}

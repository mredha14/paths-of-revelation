...
export default function Home() {
  ...
  const editPlace = async (event: React.FormEvent) => {
    ...
    setSaving(true); try { const payload=new FormData(); Object.entries(editForm).forEach(([key,value])=>payload.append(key,value)); if(editPhotoFile)payload.append('photo',editPhotoFile); const response=await fetch(`/api/places/${selected.id}`,{method:"PUT",headers:{Authorization:"Bearer "+token},body:payload}); const data=await response.json(); if(!response.ok)throw new Error(data.error); const updated={...selected,...editForm,lat:Number(editForm.lat),lng:Number(editForm.lng),photo:editPhotoFile?URL.createObjectURL(editPhotoFile):selected.photo}; setSelected(updated); setAllPlaces(current=>current.map(place=>place.id===updated.id?updated:place)); setEditOpen(false); } catch(error){setFormError(error instanceof Error?error.message:"Could not update this place.");} finally{setSaving(false);}
  };
  return (
